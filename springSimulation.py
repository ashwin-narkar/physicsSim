...
def main():
	...
	line = canvas.create_line(0,0,1,1,fill="black", dash=(4, 4))
	...
	while sim.simulate():
	    for b in objects:
	        ...
	        if newPosy >= 0:
	            canvas.move(image,delta_pos_x*10, delta_pos_y*10)
	            x = canvas.coords(image)
	            ballCenterX = (x[0]+x[2]) / 2
	            ballCenterY = (x[1] + x[3]) / 2
	            canvas.coords(line,400,550,ballCenterX,ballCenterY)
